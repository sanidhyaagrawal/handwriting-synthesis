...
import os
temp=0 
def write(text,line1,line2,line3,fontid,Username): 
                    ...
                    print('#########PAGENO###############',pageno)
                          ...
                          #  page=PC.Signle_Line_Writter(page,str(name),linenumber,space = 25, x=1100+(random.randint(2,5)*9),y1=60,font = fonts)
                           # page=PC.Signle_Line_Writter(page,str(section),linenumber,space = 25, x=1145+(random.randint(2,5)*9),y1=118,font = fonts)
                            #page=PC.Signle_Line_Writter(page,str(roll),linenumber,space = 25, x=1125+(random.randint(2,5)*9),y1=175,font = fonts)
                    if flag == 1:
                        page=PC.Signle_Line_Writter(page,str(name),linenumber,space = 25, x=1100+(random.randint(2,5)*9),y1=180-(random.randint(1,5)*3),font = fonts)
    ...
    #del(pglist[0])
    try:
         page.save(outputdir+'/temp.pdf',save_all=True, append_images=pglist)
    except:
        print("PDF file is open in another window, close it")

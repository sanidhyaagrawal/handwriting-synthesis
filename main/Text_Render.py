...
def write(text,line1,line2,line3,fontid,Username,width=55): 
    ...
    renders= os.path.dirname(os.path.realpath(__file__))[:-4]+'media/'+'renders/'    
    textfile= os.path.dirname(os.path.realpath(__file__))[:-4]+'media/'+'textfiles/'
    ...
    #new_image = Image.new("RGBA", image.size, "WHITE") # Create a white rgba background
    #new_image.paste(image, (0, 0), image)              # Paste the image on the background. Go to the links given below for details.
    pglist.append(page.convert('RGB'))
    ...
    #del(pglist[0])
    pdfname=str(datetime_object.strftime("%Y%m%d%H%M%S%f"))

    page.save(str(renders+'/'+pdfname+'.pdf'),save_all=True, append_images=pglist)
    ...
    import PyPDF2 
    pdf = renders+'/'+pdfname+'.pdf'
    outname=  renders+'/'+pdfname+'1'+'.pdf'
    print(pdf)
    ...
    PDFsplit(pdf, splits)
    os.remove(renders+'/'+pdfname+'.pdf')
    os.remove(renders+'/'+pdfname+'0'+'.pdf')
    print("Done")
